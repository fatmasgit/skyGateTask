...
function truncateText(value: string, maxLength: number = 15): string {
...
function isElixirArray(value: unknown): value is Elixir[] {
    ...
    return value.every((item: unknown): item is Elixir =>
        typeof item === "object" &&
        item !== null &&
        "id" in item &&
        "name" in item &&
            ...
            <div className="space-y-4">
                ...
                {loading ? (
                    ...
                    <>
                                        ...
                                        return (
                                            ...
                                            >
                                                {row.getVisibleCells().map((cell) => {
                                                    const cellValue: unknown = cell.getValue();
                                                    ...
                                                    if (cell.column.id === "elixirs" && isElixirArray(cellValue)) {
                                                        const visibleElixirs = isExpanded ? cellValue : cellValue.slice(0, 3);
                                                        const hasMore = cellValue.length > 3;
                                                        ...
                                                        return (
                                                            ...
                                                            >
                                                                <div className="flex flex-wrap items-center gap-2">
                                                                    {visibleElixirs.map((elixir, index) => (
                                                                        <Tooltip key={`${elixir.name}-${index}`}>
                                                                            <TooltipTrigger asChild>
                                                                                <span
                                                                                    className={`cursor-default rounded-full border px-2 py-1.5 text-[12px] font-medium ${isOddRow
                        ...
                        {!error && rows.length > 0 && pageCount > 1 && (
                            <TablePagination
                                pageCount={pageCount}
